...
class BaseController extends Controller
{
    ...
    //-- 获取分页数据
    /*------------------------------------------------------ */
    protected function getvpayuserList(&$model,&$where = '',$field = '*',$page_size = ''){
        if (empty($page_size)){
            $page_size = input("page_size/d",0);
            $session_page_size = Session::get('page_size') * 1;
            if ($page_size <= 1 ){
            ...
            elseif ($page_size != $session_page_size) Session::set('page_vapy_user_size',$page_size);
        }
        if (is_object($where) == false){//单表查询
            if (empty($this->sqlOrder)){
                $sort_by = input("sort_by/s");
    ...
    /**
     * json格式返回
     * @param  [type]  $code  1 成功 0 失败
     * @param  string  $msg   返回信息
     * @param  array   $data  返回数组
     * @param  integer $pages 返回数据分页数
     */
    protected function ajaxJson($code, $msg = '', $data = [], $pages = 1)
    {
        $return['code'] = $code;
        $return['msg']  = $msg;
        $return['data'] = $data;
