...
use think\Db;
class TurnOutPassCard extends AdminController
{

    public function initialize()
    {
        parent::initialize();
        $this->Model = new VpayTurnOutPassCard();
    }

    public function index(){
        $this->getList(true);
        return $this->fetch('sys_admin/TurnOutPassCard/index');
    }
    ...
    public function getList($runData = false, $is_ban = -1){
        ...
        if ($runData == false) {
            $data['content'] = $this->fetch('sys_admin/TurnOutPassCard/list');
