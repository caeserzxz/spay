...
//无限级分类之找子孙树
function subtree($arr,$id,$lev=1, $type = false){
    static $subs = [];//子孙数组
	if($type) $subs = null;
    foreach ($arr as $v) {
        if($v['pid'] == $id){
...
function getUserName($userId){
	$str = Users::where('user_id', $userId)->value('user_name');
	return $str;
}

// 获取用户等级
function getUserLevel($levelId){
	$str = UsersLevelModel::where('level_id', $levelId)->value('level_name');
	return $str;
}

// 获取树横线
function getTreeLine($num)
{
	$str = '| ';
	for ($i=0; $i < $num; $i++) {
		$str .= '- ';
 ...
 * vpay获取用户等级
 */
function vpayUserLevel($levelId, $returnName=true){
...
function time_section($time){
    $firstSeconds = strtotime(date("Y-m-d",$time));
//    $lastSeconds = strtotime(date("Y-m-d 23:59:59",time()));
    $lastSeconds = $firstSeconds +(3600*24);
    $arr['start'] = $firstSeconds;
    $arr['end'] = $lastSeconds;
    return $arr;
}

function monitoring_status($type,$status){
    if($type==1){
        if($status==1){
            $str = '待匹配中';
        }elseif($status==2){
