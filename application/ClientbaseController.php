...
class ClientbaseController extends BaseController{
    ...
    protected $allowAllAction = [
        ...
        'vpay/login/index', // 登录页面
        'vpay/login/register', // 注册页面
        'vpay/login/appdownload', // APP下载
        'vpay/login/changepassword', // 修改密码  忘记密码
        'publics', //前台公共页
    ];
    ...
    public function initialize()
    {
        ...
        $appstr=$_SERVER["QUERY_STRING"];
        $arr=explode("=", $appstr);
        $appType = $arr[count($arr)-1];
        if($appType=='Android'||$appType=='IOS'){
            Session::set("appType",$appType);
        }
    ...
    protected function checkLogin($isAllow = true){
        ...
        if (empty($this->userInfo)){
            ...
            return $this->redirect('vpay/Login/index');
        }
